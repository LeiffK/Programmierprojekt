<template>
  <section class="card">
    <!-- Header mit Toggle -->
    <div class="header" @click="toggle()" role="button" tabindex="0"
...
.chev{ display:inline-block; transform: rotate(0deg); transition: transform var(--dur-quick,var(--dur-soft)) var(--ease-quick,var(--ease-soft)); }
...
.tr{
  display:grid;
  grid-template-columns: 1.2fr .9fr .5fr .8fr .8fr .9fr .8fr 1.2fr;
  cursor: pointer;
...
.fade-slide-enter-active, .fade-slide-leave-active{
  transition: opacity var(--dur-soft,.22s) var(--ease-soft,cubic-bezier(.22,.61,.36,1)), transform var(--dur-soft,.22s) var(--ease-soft,cubic-bezier(.22,.61,.36,1));
}
