  ...
  <section class="card">
    ...
    <div class="form-grid">
      ...
      <div class="field col-4">
        ...
        <input v-model.number="local.seed" type="number" min="0" class="control" />
      </div>
    </div>

    <div class="row">
      <button class="btn" @click="randomize" :disabled="busy">Zufalls-Parameter (μ/σ)</button>
      <button class="btn primary" @click="init" :disabled="busy">Environment initialisieren</button>
      <div class="muted" v-if="envId">Env: <code>{{ envIdShort }}</code></div>
    </div>
  </section>
</template>
...
.group-btn{
  width:44px; height:42px; /* -2px wirkt optisch mittig innerhalb der Border */
  margin:0; padding:0;
  background:#1a1a1a;
  color:#fff;
  border:0;
