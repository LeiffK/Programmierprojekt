<template>
  <section class="card" v-if="snapshot">
    ...
    <div class="badges">
      <span class="badge">Typ: <b>{{ snapshot.config.type }}</b></span>
      <span class="badge">Arms: <b>{{ snapshot.config.arms }}</b></span>
      <span class="badge">Optimal: <b>{{ snapshot.optimalAction }}</b></span>
    </div>
    ...
    <div style="margin-top:12px">
      ...
      <div class="grid" style="grid-template-columns: repeat(3, 1fr); gap:10px">
        <div v-for="i in snapshot.config.arms" :key="'dbg-'+i" class="dbg-cell">
          <div><b>{{ String.fromCharCode(64 + i) }}</b></div>
...
<style scoped>
.dbg-cell { border:1px solid #2a2a2a; border-radius:8px; padding:8px 10px; background:#1a1a1a }
