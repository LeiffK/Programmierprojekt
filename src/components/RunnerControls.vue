<!-- src/components/RunnerControls.vue -->
<template>
  <section class="card">
...
/* Runner-Events zentral loggen */
const off = algorithmsRunner.on((msg) => {
  switch (msg.type) {
    case "READY":
      statusText.value = "Bereit";
      debugBus.info("runner", "Runner bereit.");
      break;
    case "CONFIGURED":
      ...
      debugBus.event(
        "runner",
        `Konfiguriert: Ziel ${msg.payload.totalSteps}, Rate ${msg.payload.rate}/s`,
      );
      break;
    case "STARTED":
      running.value = true;
      statusText.value = "Läuft";
      debugBus.event("runner", "Lauf gestartet.");
      break;
    case "PAUSED":
      running.value = false;
      statusText.value = "Pausiert";
      debugBus.event("runner", "Pausiert.");
      break;
    case "STOPPED":
      running.value = false;
      statusText.value = "Gestoppt";
      debugBus.warn("runner", `Lauf angehalten. ${msg.payload.reason}`);
      break;
    case "RESULT": {
    ...
    case "LOG":
      debugBus.info("runner", msg.payload.message);
      break;
    case "ERROR":
      debugBus.error("runner", msg.payload.message);
      break;
  }
});
...
/* Statusklassen für Header-Badge und FAB */
...
async function onConfigure() {
  if (!props.envId) {
    ...
    debugBus.warn("runner-ui", "Kein Environment vorhanden.");
    return;
  }
...
function onStep() {
  if (!configured.value) {
    debugBus.warn("runner-ui", "Bitte zuerst konfigurieren.");
    return;
  }
  if (running.value) {
    debugBus.warn(
      "runner-ui",
      "Während des Laufs sind Einzelschritte gesperrt.",
    );
    return;
  }
  if (currentStep.value >= totalSteps.value) {
    debugBus.info(
      "runner-ui",
      "Ziel erreicht. Bitte neu konfigurieren oder n erhöhen.",
    );
    return;
  }
...
/* Play/Pause: runder Button (YT-rot idle, Blau running) */
.fab {
  --yt-red: #ff0000;
  --run-blue: #2563eb;
  --pause-amber: #f59e0b;
