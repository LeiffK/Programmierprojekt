...
function ensureWorker() {
  if (worker) return;
  worker = new Worker(new URL("../workers/banditWorkers.ts", import.meta.url), { type: "module" });
...
function onConfigure() {
  ...
  worker!.postMessage({
    type: "CONFIGURE",
    payload: { envId: props.envId, totalSteps: totalSteps.value, rate: rate.value },
  });
}
...
.placeholder{ /* einfach leer lassen, tut nur fürs Layout */ }
