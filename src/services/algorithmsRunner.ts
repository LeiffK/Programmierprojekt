...
const DEFAULTS: Record<PolicyId, { label: string; color: string; cfg?: iBanditPolicyConfig }> = {
    greedy:    { label: "Greedy",         color: "#4fc3f7", cfg: {} },
    epsgreedy: { label: "Epsilon-Greedy", color: "#f39c12", cfg: { epsilon: 0.1 } },
};
...
export class AlgorithmsRunner {
    ...
    stepAll() {
        for (const it of this.items.values()) {
            ...
            it.history.push({ action: res.action, reward: res.reward, isOptimal: res.isOptimal });
        }
    }
