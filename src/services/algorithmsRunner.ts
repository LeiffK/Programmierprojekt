...
export type RunnerConfig = {
  ...
  totalSteps: number;
  rate: number; // Schritte/Sekunde (fachliche Rate)
...
class AlgorithmsRunner {
  ...
  // UI-Entkopplung: 60 Hz Timer + Rate-Akkumulator -> konstante Callback-Frequenz
  private static readonly UI_HZ = 60;
  private rateAcc = 0;
  ...
  // Setup Policies + Envs, kein Autolauf
  ...
  // Autolauf (Timer mit 60 Hz, Steps per Tick per Akkumulator)
  start() {
    ...
    if (this.step >= this.totalSteps && this.totalSteps > 0) {
      this.step = 0;
    ...
    const intervalMs = Math.floor(1000 / AlgorithmsRunner.UI_HZ);
    this.clearTimer();
    this.tick = window.setInterval(() => {
      if (this.status !== "RUNNING") return;

      // Rate -> Schritte pro UI-Tick aufsummieren (präzise auch für ungerade Raten)
      this.rateAcc += this.rate / AlgorithmsRunner.UI_HZ;
      const stepsThisTick = Math.floor(this.rateAcc);
      if (stepsThisTick <= 0) return;
      this.rateAcc -= stepsThisTick;
  ...
  // Einzelschritt für alle Policies (manuell/PAUSE)
  stepOnce() {
    if (!(this.status === "CONFIGURED" || this.status === "PAUSED")) {
      this.emit({
        type: "ERROR",
