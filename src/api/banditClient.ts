...
export async function initEnv(cfg: iEnvConfig): Promise<InitEnvResponse> {
    ...
    if (DEBUG) console.debug("[env:init]", { envId, cfg: env.config, optimal: env.optimalAction });

    return { envId, optimalAction: env.optimalAction };
}

export async function pullAction(envId: string, action: number): Promise<iPullResult> {
    ...
    if (DEBUG) console.debug("[env:pull]", { envId, action, res, counts: rec.counts, estimates: rec.estimates });
