<template>
  <div class="shell">
    <header class="bar"><div class="brand">Creator Lab · Mini Runner</div></header>
...
async function onManual(a: number) {
  ...
  try {
    ...
    lastEventText.value =
        `Thumbnail ${String.fromCharCode(65 + a)} → Watchtime ${res.reward.toFixed(0)}s · optimal: ${res.isOptimal ? "ja" : "nein"}`;
    refresh();
  } catch (e: any) {
