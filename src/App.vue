    ...
    <main class="wrap">
      <div class="page-grid">
        ...
        <div class="col-right">
          ...
          <!-- Zentrales Debug-Panel (ein-/ausblendbar) -->
          <DebugPanel class="card" v-model:open="debugOpen" />
...
/* Helpers */
function forwardEnvLog(msg: string) {
  debugBus.info("env", msg);
}
function onModeChange(v: "manual" | "algo") {
  debugBus.event(
    "ui",
    `Modus gewechselt: ${v === "manual" ? "Manuell" : "Algorithmus"}`,
  );
}
function refresh() {
...
function onInited({
  ...
  debugBus.event("env", "Environment initialisiert", {
    envId: id,
    optimalAction,
  });
}
...
async function onManual(a: number) {
  ...
  try {
    ...
    debugBus.event(
      "manual",
      `Klick auf Thumbnail ${String.fromCharCode(65 + a)}`,
      res,
    );
    ...
    debugBus.error("manual", e?.message ?? String(e));
    console.error(e);
  } finally {
    busy.value = false;
  }
...
/* Runner-Events → Algorithmen-Historie, rAF-drosseln */
